...
export const getPosts = async () => {
  const accessToken = Cookies.get('accessToken');
  const response = await axios.get(`${BASE_URL}/post/posts?page=1&pageSize=100000`,  {
    headers: {
      Authorization: `Bearer ${accessToken}`
    }
  });
...
export const getList = async (pagOpts: { userId: number }) => {
  const accessToken = Cookies.get('accessToken');
  const currentUser = JSON.parse(Cookies.get('user') ?? '');
  const isOwner = currentUser.userId !== pagOpts.userId ? 1 : 0;
  const response = await axios.get(`${BASE_URL}/post/list/${pagOpts.userId}?page=1&pageSize=100000&isOwner=${isOwner}`,  {
    headers: {
      Authorization: `Bearer ${accessToken}`
    }
  });
